...
impl Connection {
    ...
    where F: Fn() + Sync + Send + 'static {
        ...
        let sending = thread::spawn(move || {
            while let Ok(ConnMsg::Raw(message)) = rx.recv() {
                println!("<- {}", &message);
...
struct Core {
    ...
    transfer:   Arc<Mutex<AtomicSender<ConnMsg>>>,
}

impl Core {
    fn handle_ping(&self, message: &Object) {
        self.transfer.lock().unwrap()
            .send(ConnMsg::Raw(Pong::text(message.id())))
            .unwrap();
    }
...
struct Methods {
    outgoing:        Arc<Mutex<AtomicSender<ConnMsg>>>,
    pending_methods: HashMap<String, MethodCallback>,
    rng: Random,
}

impl Methods {
    fn new(outgoing: Arc<Mutex<AtomicSender<ConnMsg>>>) -> Self {
        Methods {
            rng:             Random::new(),
    ...
    where C: FnMut(Result<&Ejson, &Ejson>) + Send + 'static {
        ...
        self.outgoing.lock().unwrap().send(ConnMsg::Raw(method)).unwrap();
...
impl Subscriptions {
    ...
    fn sub(&mut self, name: &str, id: &mut Option<String>) {
        ...
        if let &mut Some(ref id) = id {
            ...
            self.outgoing.lock().unwrap().send(ConnMsg::Raw(sub_msg)).unwrap();
        }
    }

    fn unsub(&mut self, id: &str) {
        let unsub_msg = Unsubscribe::text(id);
        self.outgoing.lock().unwrap().send(ConnMsg::Raw(unsub_msg)).unwrap();
    }
...
impl ConnectionHandle {
    pub fn stop(self) {
        self.transfer.lock().unwrap().send(ConnMsg::Stop).ok();
